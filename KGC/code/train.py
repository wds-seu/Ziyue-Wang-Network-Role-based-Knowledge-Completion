...
settings = settings_reader.read("E:\NRKC-master\settings\gcn_basis.exp")
print(settings)
...
dataset = "E:\NRKC-master\data\DB_book"

edges_path = dataset + '/edges.dict'
nodes_path = dataset + '/nodes.dict'
...
train_triplets = io.read_triplets_as_list(train_path, nodes_path, edges_path)

valid_triplets = io.read_triplets_as_list(valid_path, nodes_path, edges_path)
test_triplets = io.read_triplets_as_list(test_path, nodes_path, edges_path)
...
nodes = io.read_dictionary(nodes_path)
edges = io.read_dictionary(edges_path)

'''
shuffled_rels = np.arange(len(edges))
np.random.shuffle(shuffled_rels)

known_rels = shuffled_rels[:int(len(edges)/2)]
target_rels = shuffled_rels[int(len(edges)/2):]
...
general_settings.put('EntityCount', len(nodes))
print("node:"+str(len(nodes)))
general_settings.put('RelationCount', len(edges))
print("edgetype:"+str(len(edges)))
general_settings.put('EdgeCount', len(train_triplets))
print("edge:"+str(len(train_triplets)))
